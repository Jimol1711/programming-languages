...
<s-expr> ::= <num>
           ...
           | <sym>
           | (list <sym> <s-expr>)*)
           | (list 'with-fun <s-fundef> <s-expr>)

<s-fundef> ::= (list 'define <sym> <sym>* <expr>)
parse-fundef ::= <s-fundef> -> Fundef
|#

(define (parse-fundef s-fundef)
  (match s-fundef
    [(list 'define fname fargs ... fbody)
     (fundef fname fargs (parse fbody))])
  )
...
(define (parse s-expr)
  (match s-expr
    ...
    [(list 'with-fun s-fundef body) (with-fun (parse-fundef s-fundef) (parse body))]
    [(list f-name f-args ...) (app f-name (map parse f-args))]))
...
(define (interp expr f-list env)
  (match expr
    ...
    [(with-fun fundef fbody) (interp fbody (cons fundef f-list) env)]
    [(app fname arg-exprs)
     (def (fundef _ fargs fbody) (lookup fname f-list))
     (def new-env (foldl (lambda (farg arg-expr fenv)
                           (extend-env farg (interp arg-expr f-list env) fenv))
                         empty-env
                         fargs
                         arg-exprs))
